...
async def analyze_subsidy_pdf(pdf_url: str) -> Dict[str, Any]:
    ...
    result = call_gemini(prompt, model="gemini-2.5-pro", system_instruction=PDF_ANALYSIS_PROMPT)
    if result:
        try:
            import re
...
def verify_document_with_ai(file_path: str, expected_company_name: str) -> Dict[str, Any]:
    """書類ファイルをAI（Gemini）で解析し、不備をチェックする"""
...
書類の内容を詳細に分析し、不備（社名の誤植、日付の期限切れ等）がないか報告してください。"""
    ...
    result = call_gemini(prompt, model="gemini-2.5-flash", system_instruction=DOCUMENT_VERIFY_PROMPT)
    if result:
        try:
            import re
            json_match = re.search(r'\{[\s\S]*\}', result)
